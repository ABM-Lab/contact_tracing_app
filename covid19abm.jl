...
const popsize = 10000 # We define the size of the population being considered in the simulation.
const initialinf = 1 # This parameter defines the number of infected individuals when the simulation starts.
const prov = :ontario
export ModelParameters, HEALTH, Human, humans, time_between_tests, popsize, initialinf, prov
...
function runsim(simnum, ip::ModelParameters)
    ...
    R01 = length(findall(k -> k.sickby in hh1,humans))/length(hh1) #Get the R0    
    ...
    vector_dead=vector_ded,R0 = R01, giso = giso, u1 = use_1, u2 = use_2, 
    quar_tot = quar_tot)
end
export runsim
...
function main(ip::ModelParameters,sim::Int64)
    ...
    popsize == 0 && error("no population size given") # Ensuring that a valid population has been given.
    hmatrix = zeros(Int16, popsize, p.modeltime) # Records the health status of each individual every day of the simulation.
    initialize() # initialize population
    # split population in agegroups 
    grps = get_ag_dist()
    insert_infected(LAT, initialinf, 4) #insert one infected in the latent status in age group 4
...
function reset_params(ip::ModelParameters)
    ...
    resize!(humans, popsize) # Creates the population.
end
...
function _get_model_state(st, hmatrix)
    ...
    for i=1:length(humans)
        hmatrix[i, st] = Int(humans[i].health_status)
    end    
end
...
function insert_infected(health, num, ag) 
    ...
    l = findall(x -> x.health_status == SUS && x.ag == ag, humans)
    if length(l) > 0 && num < length(l)
        h = sample(l, num; replace = false) 
        @inbounds for i in h 
            x = humans[i]
            x.dur = sample_epi_durations(x) # Determining the amount of time that an individual spends in each state.
            if health == PRE
                x.swap_status = PRE
                x.daysinf = x.dur[1]+1
                x.wentto = 1
...
function dyntrans(sys_time, grps,sim) # We use this function to prepare the population for the formulation of contacts.
    pos = shuffle(1:length(humans)) # We randomize the order in which individuals will be assessed.
    # go through every infectious person
    for x in humans[pos] # Iterating through the population using the new order.
            xhealth = x.health_status
...
function perform_contacts(x,gpw,grp_sample,xhealth)
    for (i, g) in enumerate(gpw) 
        ...
        for j in meet 
            ...
            if x.health_status in (PRE, ASYMP, INF) && y.health_status == SUS && y.swap_status == UNDEF 
                beta = _get_betavalue(x.health_status)
                if rand() < beta 
                    y.exp = y.tis   ## force the move to latent in the next time step.
                    y.sickfrom = x.health_status ## stores the infector's status to the infectee's sickfrom
                    y.sickby = y.sickby < 0 ? x.idx : y.sickby # Recording the index of the infector.
                    y.swap_status = LAT
                    y.daysinf = 0
                    y.dur = sample_epi_durations(y)
                end  
            elseif y.health_status in (PRE, ASYMP, INF) && x.health_status == SUS && x.swap_status == UNDEF
                beta = _get_betavalue(y.health_status)
                if rand() < beta
                    x.exp = x.tis   ## force the move to latent in the next time step.
                    x.sickfrom = y.health_status ## stores the infector's status to the infectee's sickfrom
